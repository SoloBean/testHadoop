...
public class DealAD extends Configured implements Tool{
    ...
    public static class MyPartitoner extends Partitioner<UserAndIdWritable, IntWritable>{
        public int getPartition(UserAndIdWritable key, IntWritable value, int numReduceTask){
            return Math.abs(key.User.hashCode() % numReduceTask);
        }
    }
